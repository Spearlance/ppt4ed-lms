...
class TestLMSUtils(BaseTestUtils):
	...
	def test_get_event_details(self):
		event_details = get_event_details(self.batch.name)
		self.assertEqual(event_details.name, self.batch.name)
		self.assertEqual(event_details.title, self.batch.title)
		self.assertEqual(event_details.start_date, getdate(self.batch.start_date))
		self.assertEqual(event_details.end_date, getdate(self.batch.end_date))
		self.assertEqual(event_details.start_time, to_timedelta(self.batch.start_time))
		self.assertEqual(event_details.end_time, to_timedelta(self.batch.end_time))
		self.assertEqual(event_details.timezone, self.batch.timezone)
		self.assertEqual(event_details.published, 1)
		self.assertEqual(event_details.description, self.batch.description)
		self.assertEqual(event_details.event_details, self.batch.event_details)
		self.assertEqual(len(event_details.courses), len(self.batch.courses))
		self.assertEqual(event_details.evaluation_end_date, getdate(self.batch.evaluation_end_date))
		self.assertEqual(len(event_details.instructors), len(self.batch.instructors))
		self.assertEqual(len(event_details.students), 2)
